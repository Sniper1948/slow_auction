...
contract PoolBidder is Ownable {
    ...
    event BidPlaced(address indexed pool, uint256 amount, address indexed bidder);
    event BidFailed(address indexed pool, uint256 amount, address indexed bidder, string reason);
    event DebugLog(string message, address pool, uint256 bidAmount, address sender, address owner, address poolUser, uint256 currentBid, address bannedUser);
    ...
    function bidOnMultiplePools(address[] calldata pools, uint256[] calldata specificAmounts) external onlyOwner {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            emit DebugLog("Calling bidOnPool", pools[i], specificAmounts[i], msg.sender, owner(), address(0), 0, address(0));
            try this.bidOnPool(pools[i], specificAmounts[i]) {
                // Bid successful, continue to next pool
            } catch Error(string memory reason) {
                emit BidFailed(pools[i], specificAmounts[i], owner(), reason);
                continue;
            } catch {
                emit BidFailed(pools[i], specificAmounts[i], owner(), "Unknown error");
                continue;
            }
        }
    }
    ...
    function _bidOnPool(address pool, uint256 specificAmount) internal {
        (ISilverFees.SnatchPerPoolData memory data,, address bannedUser,) = silverFees.snatchData(pool);
        require(bannedUser != address(this), "Contract is banned from pool");
        emit DebugLog("After snatchData", pool, specificAmount, msg.sender, owner(), data.user, data.bidAmount, bannedUser);
        ...
        if (specificAmount == HIGH_VALUE_BID_SENTINEL) {
            if (data.user != address(this)) {
                ...
                emit DebugLog("Skipping: Contract is highest bidder", pool, specificAmount, msg.sender, owner(), data.user, data.bidAmount, bannedUser);
                return;
            }
        ...
        require(agToken.balanceOf(address(this)) >= bidAmount, "Insufficient $AG balance");
        require(agToken.allowance(address(this), address(silverFees)) >= bidAmount, "Insufficient $AG allowance");

        emit DebugLog("Attempting snatch", pool, bidAmount, msg.sender, owner(), data.user, data.bidAmount, bannedUser);

        try silverFees.snatch(bidAmount, pool) {
            emit DebugLog("Snatch successful", pool, bidAmount, msg.sender, owner(), data.user, data.bidAmount, bannedUser);
            emit BidPlaced(pool, bidAmount, owner());
        } catch Error(string memory reason) {
            emit BidFailed(pool, bidAmount, owner(), reason);
            emit DebugLog(string(abi.encodePacked("Snatch failed: ", reason)), pool, bidAmount, msg.sender, owner(), data.user, data.bidAmount, bannedUser);
            revert(string(abi.encodePacked("Snatch failed: ", reason)));
        } catch {
            emit BidFailed(pool, bidAmount, owner(), "Unknown error in snatch");
            emit DebugLog("Snatch failed: Unknown error", pool, bidAmount, msg.sender, owner(), data.user, data.bidAmount, bannedUser);
            revert("Snatch failed: Unknown error");
        }
    }
