...
# --- Focused Countdown ---
FOCUSED_COUNTDOWN_TTE = 10.0 # TTE to enter high-precision countdown loop
...
SIMULATE_TTE_START = 60.0          # TTE (seconds) at which simulation will begin
...
MAX_BLOCKS_PER_SCAN__TTE = 100    # Max blocks to scan in one go if TTE is low
...
def fetch_historical_bids(w3: Web3, wallet_address: str, contract_address: str, method_id: str, start_timestamp: int, end_timestamp: int):
    logger.info(f"Fetching historical bids for {wallet_address} on contract {contract_address} between {datetime.datetime.fromtimestamp(start_timestamp)} and {datetime.datetime.fromtimestamp(end_timestamp)}")
    page = 1
    offset = 100
    all_bids = []
    ...
    while True:
        try:
            url = f"https://api.etherscan.io/v2/api?chainid=146&module=account&action=txlist&address={wallet_address}&startblock=0&endblock=99999999&page={page}&offset={offset}&sort=asc&apikey={ETHERSCAN_API_KEY}"
            ...
            if data['status'] == '1':
                ...
                for tx in transactions:
                    ...
                    if tx["to"].lower() == contract_address.lower() and tx['input'].startswith(method_id) and start_timestamp <= tx_timestamp <= end_timestamp:
